...
async def on_message(message):
    ...
    if mesaj.startswith("{"):
        ...
        try:
            ...
            if (stupidity >= 0 and stupidity <= 100 and (len(str(obj["discordid"]))<=19 and len(str(obj["discordid"]))>=17)):
                await ctx.send(requests.post("https://mantikralligi1.pythonanywhere.com/putUser",json={"discordid":obj["discordid"],"stupidity":obj["stupidity"],"senderdiscordid":ctx.author.id,"token":BOT_VOTE_TOKEN}).text)
            else:
